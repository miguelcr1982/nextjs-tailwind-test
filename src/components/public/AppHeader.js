...
function AppHeader() {
  ...
  return (
    <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 1 }} id='nav' className='sm:container sm:mx-auto'>
      ...
      <div className='z-10 max-w-screen-lg xl:max-w-screen-xl block sm:flex sm:justify-between sm:items-center py-6'>
        ...
        <div className='flex justify-between items-center px-4 sm:px-0'>
          <div>
            <Link href='/'>
              {/* {activeTheme === 'dark' ? (
                <Image src={logoDark} className='w-36 cursor-pointer' alt='Dark Logo' width={150} height={120} />
              ) : (
                <Image src={logoLight} className='w-36 cursor-pointer' alt='Dark Logo' width={150} height={120} />
              )} */}
              Logo
            </Link>
          </div>
      ...
      <div>{showModal ? showHireMeModal : null}</div>
    </motion.nav>
  );
}
